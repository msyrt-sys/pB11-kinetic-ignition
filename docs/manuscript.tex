...
\usepackage{url}
\urlstyle{tt}
\Urlmuskip=0mu plus 1mu\relax  % let \path break long filenames at / _ . in the narrow column
...
\item \textbf{Wang et al. 2026 (Wang)}\cite{wang} --- Table~1, five resonances, the 148~keV narrow resonance retained (a preprint, not yet peer-reviewed; adopted here as the primary modern cross section);
...
Table~\ref{tab:reactivity} reports $\langle\sigma v\rangle(T)$ for the three parameterizations from 100 to 600~keV. At 300~keV, \textbf{TB $= 4.37\times10^{-16}$, Wang $= 3.63\times10^{-16}$, NS $= 3.52\times10^{-16}$~cm$^3$/s}. The TB/Wang ratio is nearly flat at $\sim$1.20; the TB/NS ratio rises from 1.24 (300~keV) to 1.45 (600~keV). The TB curve agrees with the original Tentori--Belloni tabulation to within 0.4\%, and the low-temperature NS values are consistent with independent recent evaluations\cite{frontiers}, and the parameterized cross sections reproduce the measured $^{11}$B(p,$\alpha$) $S$-factor\cite{stave}. Figure~\ref{fig:xsec} shows $\sigma(E)$ and $\langle\sigma v\rangle(T)$ for all three. Our NS reactivity, obtained by direct $S$-factor integration, lies 3.9--11.4\% above the widely-used NS-2000 analytic fit over 300--600~keV (note~\cite{noteNS}). The $\sim$20\% spread in the reactivity is the single largest modeling uncertainty entering the power balance and is propagated explicitly below.
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{cross_section_comparison.pdf}
...
\item \textbf{Bremsstrahlung:} the NRL classical form with the Svensson relativistic correction, $P_B = 5.34\times10^{-31}\,n_e^2\,Z_\mathrm{eff}\,\sqrt{T_e}\,g(x)$, $x = T_e/m_e c^2$. The coefficient is evaluated in \textbf{mixed practical units} ($n_e$ in cm$^{-3}$, $T_e$ in keV, $P_B$ in W/cm$^3$), the natural unit over the 100--600~keV range; it is the standard NRL classical value\cite{nrl}, equivalent to $1.69\times10^{-32}$ for $T_e$ in eV ($n_e$ in cm$^{-3}$, $P_B$ in W/cm$^3$, as tabulated in the Formulary) and to $1.69\times10^{-38}$ in SI units ($n_e$ in m$^{-3}$, $T_e$ in eV, $P_B$ in W/m$^3$). The correction $g(x)$ contains the e--i and e--e relativistic terms (Svensson form) and was cross-checked against the recent independent analytical bremsstrahlung fit of Xie\cite{brems}, which spans the e--i and e--e contributions to $<$1\% over the relevant temperature range. (We adopt the NRL classical coefficient; the Wang 2026 power balance uses a value $\sim$3\% lower, a Gaunt/Born convention difference --- at the near-threshold thermal margin a further small modeling sensitivity, Sec.~III.D.) $Z_\mathrm{eff}$ and $n_e$ follow a single ash-inclusive definition $n_e = n_p + Z_B n_B + Z_\alpha n_\alpha$, $Z_\mathrm{eff} = \sum_i n_i Z_i^2 / n_e$.
...
The alpha$\to$proton diffusion scales as the product of the steady-state alpha density and the R-matrix factor, $D_\alpha \propto (n_\alpha/n_e)\cdot F_\mathrm{scale}$. Consequently the kinetic enhancement depends only on the single combined parameter \textbf{$p = (n_\alpha/n_e)\cdot F_\mathrm{scale}$}; we verify this collapse to machine precision (spread $=0$ across off-diagonal pairs, with the R-matrix scale applied live). In the $(n_\alpha, F_\mathrm{scale})$ map, $n_\alpha$ enters only as the amplitude of the alpha-drive term while the bulk composition ($n_e$, $Z_\mathrm{eff}$) is held at nominal, so the collapse onto $p$ is exact by construction; the alpha contribution to $n_e$ and $Z_\mathrm{eff}$ is retained in full in the combinatorial scan (Sec.~III.D). The two principal kinetic uncertainties are thus mathematically degenerate, and the ignition boundary in the $(n_\alpha, F_\mathrm{scale})$ plane is a hyperbola $p = p^*$. We find \textbf{$p^*(\mathrm{Wang}) = 0.013$} and \textbf{$p^*(\mathrm{TB}) = 0$} (TB ignites across the box). Figure~\ref{fig:sens2d} shows the resulting 2-D sensitivity map; within the conservative band (channeling efficiency $\le 0.5$) the Wang result is sub-ignition over a finite region and razor-thin elsewhere. This is an \textit{identifiability} statement: a single effective parameter, not two, governs the kinetic margin.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{sensitivity_2d.pdf}
...
\textbf{Combinatorial (steady-state).} A Sobol scan of \textbf{$4.19\times10^6$} parameter combinations over seven simultaneously-free levers (ion temperature, $T_e/T_i$, boron fraction, alpha density, channeling efficiency $\le 0.5$, tail fraction, drive efficiency, plus impurity and synchrotron losses; full ranges in Table~\ref{tab:levers}) finds, under the strict (Rider) accounting and the modern Wang cross section, \textbf{no positive net-power sample} (best case 14\% below threshold). Under the lenient accounting only 2 of $4.19\times10^6$ samples are positive, and both require \textit{every} optimistic lever simultaneously pinned at its ceiling (channeling efficiency $\approx 0.49$, forced electron cooling below the self-consistent value, near-zero synchrotron, aggressive ash removal); neither lies in the demonstrated-physics sub-box ($\eta_\mathrm{ch} \le 0.20$, $T_e/T_i \ge 0.40$, $n_\alpha/n_e \ge 0.01$), which contains zero positive samples. Both positive samples' full coordinates are tabulated in the released \path{montecarlo_*.csv}. Two independent accounting approaches bracket the conservative margin at \textbf{$-14\%$ to $-76\%$}, both negative. Figure~\ref{fig:mc} shows the net-power distribution.
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{montecarlo_hist.pdf}
\caption{Combinatorial net-power scan ($4.19\times10^6$ Sobol samples, seven free levers): distribution of $P_\mathrm{net}/P_\mathrm{brems}$ for the two cross sections and the two recirculation-accounting variants, with the ignition threshold marked. The strict-accounting Wang distribution lies entirely below threshold.}
\label{fig:mc}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=0.72\textwidth]{pulsed_timeseries.pdf}\\[6pt]
\includegraphics[width=0.72\textwidth]{pulsed_Gmap.pdf}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{radial_profiles.pdf}\\[6pt]
\includegraphics[width=\textwidth]{radial_Pnet_maps.pdf}
...
\begin{table*}
\caption{Sobol scan lever ranges for the combinatorial net-power scan (Sec.~III.D). Ranges are the actual Sobol bounds of the released \texttt{scripts/alpha\_channeling\_montecarlo.py}; the ion density $n_i$ is not a lever because $P_\mathrm{net}/P_\mathrm{brems}$ is density-independent (both $\propto n^2$), and $Z_\mathrm{imp}$ is a discrete two-value scenario. The null result ($0$ of $4.19\times10^6$) is therefore not an artifact of a narrow sampling box: even with every lever at its most optimistic bound, no positive net-power sample survives strict (Rider) accounting.}
\label{tab:levers}
\begin{ruledtabular}
\begin{tabular}{l l l p{0.46\textwidth}}
Lever & Symbol & Range & Rationale \\
\colrule
Ion temperature & $T_i$ & 150--600 keV & Reactivity-relevant window; $\langle\sigma v\rangle$ peaks near 600~keV, negligible fusion below $\sim$150~keV. \\
Temperature ratio & $T_e/T_i$ & 0.30--1.00 & Below the self-consistent value ($\sim$0.42) up to full thermalization; sub-self-consistent values are charged via $P_\mathrm{relax}$. \\
Boron fraction & $f_B$ & 0.05--0.30 & Brackets the reactivity-optimal fraction ($\sim$0.13); higher $f_B$ raises $Z_\mathrm{eff}$ and bremsstrahlung. \\
Ash fraction & $n_\alpha/n_e$ & 0.001--0.05 & From active ash removal\cite{ochs} to accumulation without active removal. \\
Channeling efficiency & $\eta_\mathrm{ch}$ & 0--0.5 & Zero to the idealized-theory ceiling; $\eta_\mathrm{ch}>0.5$ is not a demonstrated value\cite{ochs}. \\
Tail fraction & $f_\mathrm{tail}$ & 0.10--0.60 & Fraction of channeled power feeding the reactive suprathermal tail; upper end optimistic. \\
Drive efficiency & $\eta_\mathrm{drive}$ & 0.30--0.80 & Practical-to-optimistic RF/wave-drive efficiency. \\
Impurity fraction & $f_\mathrm{imp}$ & 0--0.02 & Clean plasma to a few-percent high-$Z$ wall contamination. \\
Synchrotron loss & $\xi_\mathrm{sync}$ & 0--0.30 & Synchrotron loss as a fraction of bremsstrahlung; 0 (fully reabsorbed) to 0.30. \\
Impurity charge & $Z_\mathrm{imp}$ & $\{6, 26\}$ & Carbon and iron (realistic high-$Z$ wall); since bremsstrahlung $\propto Z^2$, a single low-$Z$ would understate the penalty. \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
A specific reconciliation is warranted with Wang et al.\cite{wang}, whose evaluation of the same cross section concludes that bremsstrahlung does not preclude p-$^{11}$B ignition. At the level of a thermal fusion-to-bremsstrahlung balance we concur---with the identical parameterization our thermal ratio peaks marginally above unity ($\sim$1.01)---so the divergence is not one of cross-section physics but of accounting completeness. A bare thermal $P_\mathrm{fus}/P_\mathrm{brems}$ comparison reads a near-unity ratio as ``not precluded,'' whereas the present budget additionally charges the wave-drive and recirculation costs of any non-equilibrium enhancement ($P_\mathrm{drive}$, $P_\mathrm{relax}$, \`a la Rider) and propagates losses across the combinatorial, transient, and spatial regimes; under this fuller accounting the near-unity thermal margin becomes a fragile, threshold-level result with no positive net-power window. Wang's conclusion is therefore not contradicted but shown to be sensitive to the completeness of the loss accounting---the very sensitivity that is this work's central theme.
...
\textbf{Mehmet Enes S\"uzek:} Conceptualization; Methodology; Software; Validation; Formal analysis; Writing -- original draft; Writing -- review and editing.

\section{Data availability}

All numerical results are reproduced by the released code with a fixed environment (numpy 2.x, scipy 1.x, matplotlib 3.x). All code and data are archived at Zenodo (all-versions DOI: 10.5281/zenodo.20080162, which resolves to the latest release, v2.2), and are also available at \url{https://github.com/msyrt-sys/pB11-kinetic-ignition}. Each figure and table is generated by a named script with its CSV output:
\begin{itemize}
\item Table~\ref{tab:reactivity}, Figure~\ref{fig:xsec} --- \path{scripts/fig_cross_sections.py} (cross sections + reactivity); a $\langle\sigma v\rangle$ benchmark gate verifies the integrator to $<$0.15\%.
\item Figure~\ref{fig:sens2d} --- \path{scripts/sensitivity_analysis_2d.py} (+ \path{sensitivity_2d_grid.csv}, \path{sensitivity_2d_summary.csv}).
\item Figure~\ref{fig:mc} --- \path{scripts/alpha_channeling_montecarlo.py} (+ \path{montecarlo_*.csv}); includes a mandatory interpolation self-check ($<$0.1\%).
\item Figure~\ref{fig:pulsed} --- \path{scripts/pulsed_0d_model.py} (+ \path{pulsed_*.csv}); interpolation self-check ($<$0.1\%).
\item Figure~\ref{fig:radial} --- \path{scripts/radial_1d_model.py} (+ \path{radial_*.csv}); interpolation self-check ($<$0.1\%) and a uniform-profile $\to$ 0-D reduction self-check ($<$1\%).
\end{itemize}

\textbf{Independent verification.} The independent from-scratch reimplementation is included in the archive (\path{validation/}), sharing no code with the primary implementation (SI units, an independent integrator, the Wang/Rider bremsstrahlung form), and reproduces the principal results (reactivity to $<$0.1\%, thermal peaks, the combinatorial null, pulsed $G<1$, and transport dominance). The results are additionally anchored to published external evaluations (Tentori--Belloni 2023 cross section to 0.4\%; Nevins--Swain low-temperature reactivity; the relativistic bremsstrahlung terms\cite{brems}; and the NRL ion--electron exchange rate). The cross-section coefficients were validated three independent ways against the published Table~1.
...
\bibitem{wang} H.-Y. Wang, Y.-Q. Li, Q.~Wu, and Z.-F. Cui, ``Revisiting p-$^{11}$B Fusion: Updated Cross-sections, Reactivity, and Energy Balance,'' arXiv:2601.00241 (2026) (preprint, not yet peer-reviewed).
