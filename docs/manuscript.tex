...
\item \textbf{Bremsstrahlung:} the NRL classical form with the Svensson relativistic correction, $P_B = 5.34\times10^{-31}\,n_e^2\,Z_\mathrm{eff}\,\sqrt{T_e}\,g(x)$, $x = T_e/m_e c^2$. The coefficient is evaluated in \textbf{mixed practical units} ($n_e$ in cm$^{-3}$, $T_e$ in keV, $P_B$ in W/cm$^3$); it is the standard NRL classical value\cite{nrl}, tabulated in the Formulary as $1.69\times10^{-32}$ for $T_e$ in eV ($n_e$ in cm$^{-3}$, $P_B$ in W/cm$^3$) and rescaled here to $T_e$ in keV ($\times\sqrt{1000} = 5.34\times10^{-31}$), the natural unit over the 100--600~keV range of interest. The correction $g(x)$ contains the e--i and e--e relativistic terms (Svensson form) and was cross-checked against the recent independent analytical bremsstrahlung fit of Xie\cite{brems}, which spans the e--i and e--e contributions to $<$1\% over the relevant temperature range. (We adopt the NRL classical coefficient; the Wang 2026 power balance uses a value $\sim$3\% lower, a Gaunt/Born convention difference --- at the near-threshold thermal margin a further small modeling sensitivity, Sec.~III.D.) $Z_\mathrm{eff}$ and $n_e$ follow a single ash-inclusive definition $n_e = n_p + Z_B n_B + Z_\alpha n_\alpha$, $Z_\mathrm{eff} = \sum_i n_i Z_i^2 / n_e$.
