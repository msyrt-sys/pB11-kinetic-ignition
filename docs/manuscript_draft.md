...
Table 2 consolidates the four regimes. Four distinct modeling sensitivities are reported transparently and propagate into the margins: the NS analytic-vs-integral offset (Sec. II.A), the combinatorial accounting bracket (−14% to −76%), the pulsed alpha-deposition accounting (G ≤ 0.36), and the Coulomb logarithm (Sec. II.E).
