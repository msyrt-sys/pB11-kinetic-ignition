"""
α-p elastic scattering cross section — R-matrix phase-shift approach.
...
Approach: compute σ(E_p_lab, θ_lab) from Coulomb + nuclear phase shifts.

⁵Li energy levels (in proton CM energy):
- Ground state: 3/2- at E_p_CM = 1.97 MeV (lab E_p ≈ 9.85 MeV proton on 4He)
  Note: we use inverse kinematics here (proton on alpha).
  Standardization: E_alpha_CM = E_alpha * m_p/(m_a+m_p) = E_alpha/5
  ⁵Li resonance E_α_lab ≈ 9.85 MeV (proton on 4He);
  inverse: E_α_lab × 4 ≈ 1.97 MeV (4He on p).
  For our system (proton on alpha):
  ...
  In our plots the resonance appears at ~8 MeV alpha lab.
  This corresponds to 8/5 = 1.6 MeV CM = E_p_CM (if proton beam) = 4 × 1.6 = 6.4 MeV proton lab.
  This is a different region — most likely the P-wave 1/2- excited state of ⁵Li (E* ~ 4 MeV).

Numerical evaluation:
σ(θ) = |f_C(θ) + f_N(θ)|²
f_C: Coulomb amplitude (from Rutherford)
f_N: nuclear amplitude (from phase shifts)

Pragmatic fit: we fit a DIRECT FUNCTION σ_s/σ_R(E, θ) that reproduces
...
def coulomb_amplitude(E_CM_MeV, theta_CM_rad, Z1=1, Z2=2):
    ...
    The modulus squared yields the Rutherford cross section.
    """
...
def nuclear_amplitude_pragmatic(E_alpha_lab_MeV, theta_alpha_CM_rad):
    """Pragmatic nuclear amplitude — reproduces Belloni's σ_s/σ_R structure.

    Main features of the ⁵Li system:
    - Ground state 3/2- at E_α_lab ≈ 2.5 MeV (weak)
    - 1/2- excited state at E_α_lab ≈ 8 MeV (the resonance Belloni captures)

    This function computes an approximate f_N restricted to the l=0,1 partial
    waves, sufficient to approximately reproduce Belloni Fig. 2.
    """
    ...
    E_R1 = 1.6  # MeV CM (≈ 8 MeV alpha lab) — strong P3/2 resonance
    ...
    # Coupling to 5Li 5/2- at E_α ~ 16 MeV
...
def sigma_s_phase_shift(E_alpha_lab_MeV, theta_p_lab_rad):
    """σ_s(E_α, θ_p,lab) — phase-shift evaluation.

    1. theta_p_lab → theta_alpha_CM (kinematic)
    2. f_C + f_N total amplitude
    3. |f|² = differential cross section in CM frame
    4. Transform to lab frame (Jacobian)
    """
...
def rutherford_lab(E_alpha_lab_MeV, theta_p_lab_rad):
    """Pure Rutherford cross section in the lab frame for proton recoil."""
    E_CM = E_alpha_lab_MeV / 5
    theta_alpha_CM = np.pi - 2 * theta_p_lab_rad
...
print("Phase-shift-based σ_s/σ_R check:")
